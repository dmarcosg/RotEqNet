...
for i=1:n
  ...
  switch l.type
	case 'bnormangle'
      if testMode
        res(i+1).x = vl_nnbnormangle(res(i).x, l.weights{1}, l.weights{2}, 'moments', l.weights{3}) ;
      else
        res(i+1).x = vl_nnbnormangle(res(i).x, l.weights{1}, l.weights{2}) ;
      end
      
    case 'pool_ext'
        if size(res(i).x,5) == 1
            res(i+1).x = vl_nnpool_ext(res(i).x,res(i).x, l.pool,'pad',l.pad) ;
        else
            nor = sqrt(res(i).x(:,:,:,:,1).^2 + res(i).x(:,:,:,:,2).^2);
            res1 = vl_nnpool_ext(res(i).x(:,:,:,:,1),nor, l.pool,'pad',l.pad) ;
            res2 = vl_nnpool_ext(res(i).x(:,:,:,:,2),nor, l.pool,'pad',l.pad) ;
            res(i+1).x = cat(5,res1,res2);
        end
            
      
    case 'poolangle'
          res(i+1).x = vl_nnpoolangle(res(i).x, ...
            'bins', l.bins, 'angle_n', l.angle_n, 'max_angle', l.max_angle, l.opts) ;
    case 'convsteer'
          res(i+1).x = vl_nnconvsteer(res(i).x, l.weights{1}, l.weights{2}, ...
              'angle_n',l.angle_n,...
              'pad', l.pad, ...
              'stride', l.stride, ...
              l.opts{:}, ...
	...
	case {'normalizelp'}
      res(i+1).x = vl_nnnormalizelp(res(i).x,[], 'p', l.p, 'epsilon', l.epsilon,'norm', l.norm) ;
    ...
    case 'sigmoid_1'
      res(i+1).x = vl_nnsigmoid_1(res(i).x,'apply',l.apply) ;
    ...
    case 'pdist_siam'
      res(i+1).x = vl_nnpdist(res(i).x(:,:,:,1:2:end-1,:), res(i).x(:,:,:,2:2:end,:), l.p, ...
        'noRoot', l.noRoot, ...
        'epsilon', l.epsilon, ...
        'aggregate', l.aggregate, ...
        'instanceWeights', l.instanceWeights,...
        'classWeights', l.classWeights) ;
...
if doder
  ...
  for i=n:-1:backPropLim
    ...
    switch l.type
      case 'bnormangle'
        [res(i).dzdx, dzdw{1}, dzdw{2}, dzdw{3}] = ...
          vl_nnbnormangle(res(i).x, l.weights{1}, l.weights{2}, res(i+1).dzdx) ;
      ...
      case 'pool_ext'
        if size(res(i).x,5) == 1
            res(i).dzdx = vl_nnpool_ext(res(i).x,res(i).x, l.pool, res(i+1).dzdx,'pad',l.pad) ;
        else
            abs1 = abs(res(i).x(:,:,:,:,1));
            abs2 = abs(res(i).x(:,:,:,:,2));
            nor = sqrt(abs1.^2 + abs2.^2);
            res(i).dzdx = zeros(size(res(i).x),'like',res(i).x);
            res(i).dzdx(:,:,:,:,1) = vl_nnpool_ext(res(i).x(:,:,:,:,1),nor, l.pool, res(i+1).dzdx(:,:,:,:,1),'pad',l.pad) ;
            res(i).dzdx(:,:,:,:,2) = vl_nnpool_ext(res(i).x(:,:,:,:,2),nor, l.pool, res(i+1).dzdx(:,:,:,:,2),'pad',l.pad) ;
        end
        
      case 'poolangle'
        res(i).dzdx = vl_nnpoolangle(res(i).x,  res(i+1).dzdx, ...
                                'bins', l.bins, 'angle_n', l.angle_n, 'max_angle', l.max_angle, l.opts);
                            
      case 'convsteer'
      [res(i).dzdx, dzdw{1}, dzdw{2}] = ...
        vl_nnconvsteer(res(i).x, l.weights{1}, l.weights{2}, res(i+1).dzdx, ...
        'angle_n',l.angle_n,...
        'pad', l.pad, ...
        'stride', l.stride, ...
        l.opts{:}, ...
      ...
      case {'normalizelp'}
        res(i).dzdx = vl_nnnormalizelp(res(i).x, res(i+1).dzdx , 'p', l.p, 'epsilon', l.epsilon,'norm', l.norm) ;
      ...
      case 'sigmoid_1'
        res(i).dzdx = vl_nnsigmoid_1(res(i).x, res(i+1).dzdx,'apply',l.apply) ;
      ...
      case 'pdist_siam'
          indices = [1:size(res(i+1).dzdx,4);1:size(res(i+1).dzdx,4)];
        res1 = vl_nnpdist(res(i).x(:,:,:,1:2:end-1,:), res(i).x(:,:,:,2:2:end,:), ...
          l.p, res(i+1).dzdx, ...
          'noRoot', l.noRoot, ...
          'epsilon', l.epsilon, ...
          'aggregate', l.aggregate, ...
          'instanceWeights', l.instanceWeights,...
          'classWeights', l.classWeights) ;
        res(i).dzdx = res1(:,:,:,indices(:),:);
        sign_change = [1*ones(1,size(res(i+1).dzdx,4)); -1*ones(1,size(res(i+1).dzdx,4))];
        res(i).dzdx = bsxfun(@times,res(i).dzdx,permute(sign_change(:),[4 2 3 1]));
    ...
    switch l.type
      case {'conv', 'convt', 'bnorm','convsteer','convsteert','bnormangle'}
        if ~opts.accumulate
          res(i).dzdw = dzdw ;
        else
          for j=1:numel(dzdw)
